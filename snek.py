...
#make a menu screen, press up button to start, down to cancel?
while True:
	reset_bg()
	...
	#make font settings
	font_size = 50
	font_height = (c_height/2)-font_size
	font_width = (c_width/2)-font_size
	sub_font_height = (c_height/2)
	sub_font_width = (c_width/2)-(font_size*3.25)
	my_font = pygame.font.SysFont(pygame.font.get_default_font(),50)
	sub_font = pygame.font.SysFont(pygame.font.get_default_font(),25)
	#make the title surface with font
	text_surface = my_font.render('Snek',False,(0,0,0));
	sub_text_surface = my_font.render('press space to start',False,(0,0,0));
	screen.blit(text_surface,(font_width,font_height))
	screen.blit(sub_text_surface,(sub_font_width,sub_font_height))
	#update with menu font
	pygame.display.update()
